...
onMounted(async () => {
  await authGet('/Survey/GetSummary').then((response) => {
    const tableArray = response.data.data;

    sample.value = tableArray.sampleSize;
...
const importish = () => {
  ...
  authPost('/Survey/Load', formData, {
    'Content-Type': 'multipart/form-data'
  });

  authGet('/Survey/GetSummary').then((response) => {
    const tableArray = response.data.data;
...
<template>
  <q-card>
    ...
    <q-card-section class='column items-start' style='row-gap: 16px'>
      ...
      <div class='row fit justify-end no-wrap'>
        <q-btn
          @click='importChanges'
