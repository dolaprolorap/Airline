...
const airportListFrom = ref('All airports');
...
const airportListTo = ref('All airports');
...
const copyrows: Ref<Row[]> = ref([]);

const selectedFlights: Ref<Row[]> = ref([]);
...
const applyFilters = () => {
  ...
  let filteredRows = rows.value.filter((row) => {
    const fromFilter =
      airportListFrom.value !== 'All airports'
        ? row.from === airportListFrom.value
        : true;
    const toFilter =
      airportListTo.value !== 'All airports'
...
<template>
  <q-page class='row items-center justify-center' style=''>
    <q-card>
      ...
      <q-card-section class='q-pb-none column items-end'>
        <span class='text-primary text-h6'>Filters</span>
        <q-card-section class='q-pa-none fit row items-center q-pb-none justify-between'>
      ...
      <q-card-section class='column items-start' style='row-gap: 16px'>
        <q-table
          ...
          :rows-per-page-options='[20, 10, 15, 25, 50, 0]'
        >
          ...
          <template v-slot:body='props'>
            <q-tr
              ...
              :class='props.row.active ? "" : "bg-red text-white"'
            >
              <q-td>
                <q-checkbox
